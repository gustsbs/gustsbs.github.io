...
### Definir a flag 'noout' no Ceph
Impede que o Ceph tente replicar os OSDs do nó que sairá do ar, evitando sobrecarga na rede
 ```bash
 ceph osd set noout
 ```

### Monitorar em tempo real a migração e o estado do HA
```bash
watch -n 2 ha-manager status
```

### Listar as VMs rodando localmente no nó saudável para confirmar a aterrissagem
```bash
qm list
```

### Desativar o modo de manutenção no pve1 (reintegrando o nó ao HA do cluster)
```bash
